...
- formatter ([SqlFormatter](SqlFormatter.md)) SqlFormatter associato al db
- sqlConn ([Connection](Connection.md)) connessione fisica al database
...
- security  ([Security](jsSecurity.md))
- getDataInvoke ([GetDataInvoke](GetDataInvoke.md)): classe con cui si possono leggere dati dal db con
 un'interfaccia omogenea, sia che si stia eseguendo le istruzioni lato server che lato client
- getDataSet ([GetDataSet](GetDataSet.md)): classe che espone dei metodi per creare un dataset (da usare solo lato server)
- getMeta ([GetMeta](GetMeta.md)): classe tramite la quale si può istanziare un metadato
