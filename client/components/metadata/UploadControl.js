...
(function() {
     ...
     * @constructor
     */
    function UploadControl(el, helpForm, table, primaryTable, listType) {
    ...
    UploadControl.prototype = {
        ...
        uploadCore:function (file) {
            ...
            var filename = self.lblFileName || file.name;
        ...
        download:function (that) {
            ...
            // se è impostato dal programamtore, altriemnti metto quell'utnete, e se non lo trovo metto quello del DS, e altriemnto metto stringa fissa
            var filename = that.lblDownloadFileName;
            if (!filename){
                var file = $("#" + that.idUploadFile).get(0).files[0];
                // se c'è bindato file
                if (file) filename = file.name;
                // se non c'è il file dell'utnete vedo se posso ricavare dal DS
                if (!file){
                    var dtattach = that.metaPage.state.DS.tables[that.tableAttachName];
                    var rowsReferenced = dtattach.select($q.eq(that.idAttachColumnName, idAttach ));
                    if (rowsReferenced.length) filename = rowsReferenced[0][that.fileNameColumnName];
                }
            }
            // se non trovo metto stringa fissa
            if (!filename) filename = localResource.attachment;
            filename = that.getOriginalFileName(filename);

            // mostro indicatore di attesa
            var waitingHandler = that.metaPage.showWaitingIndicator(localResource.downloadAttachWaiting);
            // chiamo metodo per il download
			that.attachManger.download(idAttach, filename)
				.then(function () {
					// nascondo indicatore di attesa
         ...
         * @public
         * @description ASYNC
         * Remove tge attachment for the main row of the form.counter on dt attach.
         * @param {UploadControl} that
         */
        removeAttachment:function (that) {
            ...
            // Nel momento in cui salverò il save del server si accorge che una riga con attach è modified e idattach è nullo , quindi si occuperà lei di mettere il counter a zero
        ...
        fillControl:function(el){
            ...
            var dtattach = this.metaPage.state.DS.tables[this.tableAttachName];
            if (!dtattach) return this.metaPage.showMessageOk(localResource.getMissingTableDataSet(this.tableAttachName));
            this.setLblFileName(idattachValue, dtattach);
    ...
    window.appMeta.CustomControl("upload", UploadControl);
