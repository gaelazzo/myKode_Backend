...
E' possibile integrarne o modificare i vari metodi per customizzarla in base alle proprie esigenze.

### Autenticazione
L'autenticazione è gestita mediante token [jwt](https://jwt.io/introduction). Ad ogni utente è associata una sessione
ed è assegnato un'id di sessione, che poi è inserito nel token criptato jws.

La jsApplication si occupa di gestire le sessioni utente, di creare un contesto([Context](Context.md)) per ogni richiesta
...
Si occupa anche di creare le routes e di garantirne l'accesso solo se si fornisce un token adeguato nell'header
...
La cartella auth è infatti usata per le routes che servono ad autenticarsi. 

Anche le richieste non autenticate devono fornire un token fittizio, "anonimo", il cui valore si configura
nel file config/tokenConfig nella proprietà AnonymousToken.

Pertanto nell'header della richiesta http dovrà esserci in ogni caso una riga del tipo
...
Il token è autenticato e decodificato nel campo request.**auth**, configurabile in 
tokenConfig.options.requestProperty. Dunque in generale request.auth ci sarà un oggetto di tipo [Token](src/jsToken.js)
...
- [DataAccess](DataAccess.md) è usata per operazioni di basso livello, come inviare semplici comandi di lettura o scrittura
