...
describe('GetData', function () {
    ...
    beforeEach(function () {
        ...
        $("body").append('<link rel="stylesheet" href="base/test/client/app/styles/app.css" />');
    });
            ...
            it('1. getDataSet empty from server, 2. invoke preFillDataSet on one table 3. ds should be correctly formatted and filled',
                function(done) {
                    defLogin.then(function () {
                        ...
                        conn.call(objConn)
                            .then(function(res) {
                                    logger.log(logType.INFO, "Restituito ds vuoto");
                                    ...
                                    // N.B la tab registryreference e registry non inserisco nel dict
                                    // quindi non voglio caricare i dati

                                    appMeta.getData.prefillDataSet(ds1, "registry", "anagrafica")
                                        .then(function(dsTarget) {
                                                logger.log(logType.INFO, "Restituiti i dati della prefill");
                                                ...
                                                if (tRegistryAddress.rows.length === 0) {
                                                    logger.log(logType.ERROR,
                                                        "Non sono state restituite righe, il test 1 '" +
                                                        //spec1.description +
                                    ...
                                    appMeta.getData.prefillDataSet(ds1, "registry", "anagrafica")
                                        .then(function(dsTarget) {
                                                ...
                                                if (tRegistry.rows.length === 0) {
                                                    logger.log(logType.ERROR,
                                                        "Non sono restituite righe, il test 2'" +
                                                        //spec2.description +
                                                        "' non è Attendibile con questi dati");
                                                }
                                                ...
                                                if (tRegistryAddress.rows.length === 0) {
                                                    logger.log(logType.ERROR,
                                                        "Non sono sono state restituite righe, il test 2'" +
                                                        //spec2.description +
                                                        "' non è Attendibile con questi dati");
                                                }
                    ...
                    defLogin.then(function () {
                        ...
                        conn.call(objConn)
                            .then(function (res) {
                                ...
                                ds1 = appMeta.getDataUtils.getJsDataSetFromJson(res);

                                // costruisco selBuilderArray
                                var selBuilderArray = new Array();
                                selBuilderArray.push({ filter: $q.eq("lu", "sa"), top: 300, tableName: "registryreference", table: ds1.tables['registryreference']});
                                selBuilderArray.push({ filter: $q.eq("cu", "assistenza"), top: 100, tableName: "registry", table: ds1.tables['registry']});
                                selBuilderArray.push({ filter: $q.and($q.eq("cap", '00100'),$q.eq("lu", "user")) , top: 100, tableName: "registryaddress", table: ds1.tables['registryaddress']});


                                getData.multiRunSelect(selBuilderArray)
                                    .then(
                                        function () {
                                            expect(ds1.tables["registry"].rows.length).toBeGreaterThan(0);
                                            expect(ds1.tables["registryaddress"].rows.length).toBeGreaterThan(0);
                                            expect(ds1.tables["registryreference"].rows.length).toBeGreaterThan(0);
                                            done();
                                        })
                                    .fail(
                                        function (err) {
                                ...
                                getData.doGetTable(t, filter, true, 10, selectList)
                                    .then(function () {
                                        ...
                                        expect(appMeta.getDataUtils.getJsonFromJsDataQuery(selectList[1].filter))
                                            .toEqual(appMeta.getDataUtils.getJsonFromJsDataQuery(composedFilter));
                    ...
                    defLogin.then(function () {
                        ...
                        conn.call(objConn)
                            .then(function(res) {
                                ...
                                tRegistryAddress.staticFilter($q.and($q.eq("cap", '00100'),$q.eq("lu", "user")));
            ...
            it('selectCount() is ASYNC returns an integer, table registry, filtered on "cu" field',
                function(done) {
                    defLogin.then(function () {
                        ...
                        getData.selectCount("registry", filter).then(function (res) {
