...
describe('ConnectionWebService No Auth', function () {
    ...
    describe("ConnectionWebServiceNoAuth class",
        function () {
            ...
            it("User reset Password, gets error 'Email not registered'", function (done) { //Utente già registrato, ma non ancora attivo su web
                var objConn = {
            ...
            it("Login + httpCall", function (done) {
                appMeta.authManager.login(
                    ...
                    .then(function (res)  {
                        ...
                        let filter = $q.eq($q.field("cu"), "assistenza");
                        ...
                        conn.call(objConn)
                            .then(function (res) {
                                    let dt = appMeta.getDataUtils.getJsDataTableFromJson(res);
                                    expect(dt.name).toBe("registry"); // nome tabella
                                    expect(dt.rows.length).toBe(3); // top tre righe
                                    expect(Object.keys(dt.columns).length).toBe(3); // solo le 3 colonne richieste
            ...
            xit('runSelect() , columnList ="idreg,active,cu", must FAILS, because it requires logged user',
                function (done) {
                    ...
                    appMeta.getData.runSelect('registry', columnList, filter, top)
                        .then(function (dt) {
                            // il metodo deve fallire se torna con successo, poiché deve essere richeista la login
                            expect(true).toBe(false);
                            done();
                        },
                            function(err) {
                                //  il test è ok se va in errore poiché non ho le credenziali.
