    ...
    afterAll(() => {
        appMeta.connection.setTestMode(true); //shows the modal window
            ...
            it('Multirunselect Send jsDataQuery to server, basic test: Function "Eq" + "Field" undefined value in integer non nullable',
                function (done) {
                    defLogin.then(function () {
                        ...
                        selBuilderArray.push({ filter: $q.eq($q.field("lu"), undefined), top: 300, tableName: "registryreference", table: t1 });
                        selBuilderArray.push({ filter: $q.eq($q.field("cu"), "assistenza"), top: 100, tableName: "registry", table: t2 });
                        selBuilderArray.push({ filter: $q.and($q.eq($q.field("cap"), undefined), $q.eq($q.field("lu"), "Vercelli")), top: 100, tableName: "registryaddress", table: t3 });
                    ...
                    defLogin.then(function () {
                        ...
                        conn.call(objConn)
                            .then(function (res) {
                                ...
                                expect(_.isEqual(m.myArguments[1][0], "a")).toBe(true);
                                done();
                            },
                            function (err) {
                                //logger.log(logType.ERROR, 'Errore jsDataQuery ', 'err: ' , err);
            ...
            it('Send jsDataQuery to server, basic test: Function "lt" on field type Date, select on t1 is ok',
                function (done) {
                    defLogin.then(function () {
                        ...
                        // da spedire quindi applico offset per evitare lo stringify
                        var mydateToSend = new Date(2018, 0, 25, 15, 7); // appMeta.getDataUtils.normalizeDataWithoutOffsetTimezone(new Date(2018, 0, 25, 15, 7), true);
            ...
            it('Send jsDataQuery to server, basic test: Function "le" on field type Date, select on t1 is ok',
                function (done) {
                    defLogin.then(function () {
                        ...
                        var mydateToSend = mydateClient; // appMeta.getDataUtils.normalizeDataWithoutOffsetTimezone(new Date(2018, 0, 25, 15, 7), true);
                    ...
                    defLogin.then(function () {
                        ...
                        return conn.call(objConn)
                            .then(function (res) {
                                ...
                                var t1filter2 = t1.select(m); // serve per confrontare
                                expect(_.isEqual(t1filter1, t1filter2)).toBe(true);
                                // verifico che siano i record aspettati, sono 4 gli input e quindi vale la pena vedere se funzionao i < > <= >= 
            ...
            it('Send jsDataQuery to server, basic test: Function "And(gt, lt)" on field type Date, select on t1 is ok',
                function (done) {
                    defLogin.then(function () {
                        ...
                        // da spedire quidni applico offset epr evitare lo stri gfy
                        var mydateToSend1 = mydateClient1; // appMeta.getDataUtils.normalizeDataWithoutOffsetTimezone(new Date(2018, 0, 25, 15, 7), true);
                        var mydateToSend2 = mydateClient2; // appMeta.getDataUtils.normalizeDataWithoutOffsetTimezone(new Date(2018, 0, 25, 15, 9), true);
            ...
            it('Send jsDataQuery to server, basic test: Function "And(gt, lt)" on field type Date, sql string is expected',
                function (done) {
                    defLogin.then(function () {
                        var mydateToSend1 = new Date(2018, 0, 25, 15, 7);
                        var mydateToSend2 = new Date(2018, 0, 25, 15, 9)
                        var inputFilter = $q.and($q.gt('c_date', mydateToSend1), $q.lt('c_date', mydateToSend2));

                        //{ "filter": { "name": "and", "args": [{ "name": "gt", "args": 
                        //      [{ "value": "c_date" }, { "value": "2018-01-25T14:07:00.000Z" }]}, 
                        //  { "name": "lt", "args": [{ "value": "c_date" }, { "value": "2018-01-25T14:09:00.000Z" }] }]
                        //  }, "idRequest": 2 }
            ...
            it('Send jsDataQuery json of doPar serialized',
                function (done) {
                    defLogin.then(function () {
                        var j2 = {"name":"doPar","args":[{"name":"or",
                                "args":[{"name":"doPar","args":[{"name":"and","args":
                                                [{"name":"eq",
                                                        "args":[{"name":"field","args":[{"value":"a"}],
                                                            "alias":"a"},
                                                            {"name":"constant","args":[{"value":111}]}]},
                        {"name":"eq","args":[{"name":"field","args":[{"value":"b"}],"alias":"b"},
                                {"name":"constant","args":[{"value":222}]}]}]}]},
                                    {"name":"doPar","args":[{"name":"eq","args":[{"name":"field","args":[{"value":"c"}],
                                                "alias":"c"},{"name":"constant","args":[{"value":333}]}]}]}]}]};
            ...
            it('Send jsDataQuery to server, basic test: Function "isNotIn", sql string is expected',
                function (done) {
                    var inputFilter = $q.isNotIn('c_dec', [11, 33]);
                    defLogin.then(function () {
