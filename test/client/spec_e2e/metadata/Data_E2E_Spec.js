    ...
    describe("Test DataSet with server",
        function() {
            ...
            it('Take custom JsDataSet deserializes it, sends to server and gets again from the server in the answer',
                function(done) {
                    defLogin.then(function () {
                        ...
                        var jsonToSend = serMyDs; // JSON.stringify(serMyDs);
                        ...
                        conn.call(objConn)
                            .then(function (res) {
                                    ...
                                    expect(Object.keys(defaultsT1).length).toBe(2);
                                    expect(defaultsT1.c_name).toBe("Sdef");
                                    expect(defaultsT1.c_dec).toBe(1234);
                                    expect(defaultsT1.c_double).toBe(undefined);
                                    ...
                                    // altri campi senza default in partenza hanno undefined
                                    expect(defaultsT3.c_alt).toBe(undefined);
                                    expect(defaultsT3.c_sex).toBe(undefined);
                    ...
                    defLogin.then(function () {
                        ...
                        conn.call(objConn)
                            .then(function (res) {
                                    ...
                                    expect(ds1.name).toBe("dsmeta_registry_anagrafica"); // nome aspettato
                                    //_.forEach(ds1.tables['registry'].columns,x=>console.log(x.name));
                    ...
                    defLogin.then(function () {
                        ...
                        conn.call(objConn)
                            .then(function (res) {
                                    ...
                                    // il num di defaults è lo stesso del num colonne
            ...
            it('FillDataSet method with Eq filter',
                function(done) {
                    defLogin.then(function () {
                        let filter = $q.eq("cu", "assistenza");
                        let filterSerialized = appMeta.getDataUtils.getJsonFromJsDataQuery(filter);
                        ...
                        conn.call(objConn)
                            .then(function (res) {
                                    ...
                                    let ds1  = appMeta.getDataUtils.getJsDataSetFromJson(res);

                                    expect(ds1.name).toBe("registry_anagrafica");

                                    var t = ds1.tables["registry"];
                                    if ( t.rows.length === 0){
                                        logger.log(logType.ERROR, "Non sono state trovate delle righe, il test 'FillDataSet method with Eq filter' non è Attendibile con questi dati");
                                    }
            ...
            it('FillDataSet method with AND(eq, eq) filter',
                function(done) {
                    defLogin.then(function () {
                        let filter = $q.and($q.eq($q.field("cu"), "sa"), $q.eq($q.field("residence"), 3));
                        ...
                        conn.call(objConn)
                            .then(function (res) {
                                    ...
                                    expect(ds1.name).toBe("registry_anagrafica");
                                    let t = ds1.tables["registry"];

                                    if ( t.rows.length === 0){
                                        logger.log(logType.ERROR, "Non sono state trovate righe, il test 'FillDataSet method with AND(eq, eq) filter' non è Attendibile con questi dati");
                                    }
