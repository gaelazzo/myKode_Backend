(function () {
	...
	TestHelper.prototype = {
         ...
         * @param {string} pathRelativeOfTest string that indicates the relative path from spec_e2e/ for the specific MetaPage of the test
         */

        /**
         * Initializes the appMeta for the production apps.
         * @param {string} dbCode,  dbCode for service path
         */
		initAppTestProduction: function (dbCode) {
			jasmine.getFixtures().fixturesPath = "/main/";
			jasmine.getFixtures().load('index_e2e.html');
			...
			//console.log("resetting appMeta.appMain ***************")
			//window.appMeta.appMain = {}; // inizializza oggetto appMain che in test non verrebbe tirato su

			//nell'ambiente di Quality Assurance attivo il log al livello debug altrimento lascio quello di produzione ERROR
			window.appMeta.logger.levelLog =  window.appMeta.logTypeEnum.ERROR;

			window.appMeta.basePath = 'base/';
			window.appMeta.serviceBasePath = "/"+dbCode+"/"; // path relativo dove si trovano i servizi
			window.appMeta.config.backendType = 'node';
			window.appMeta.routing.backendUrl = "http://localhost:54471";
			window.appMeta.config.env = appMeta.config.envEnum.QA;
			// rebase del path poiché i file necessari per il test, in particolare la MetaPage derivata sta sotto base/<pathapp>
			window.appMeta.basePathMetadata = 'metadata/';
			//console.log("assigned basePathMetadata = "+ window.appMeta.basePathMetadata);
			window.appMeta.config.path_maintoolBarTemplate = "components/userTemplate/mainToolBar_Template.html";
			window.appMeta.config.defaultDecimalFormat = 'n';
			window.appMeta.config.defaultDecimalPrecision = 2;
			window.appMeta.config.dataContabileYear = (new Date()).getFullYear();
			window.appMeta.config.codiceDipartimento = 'amministrazione';
			window.appMeta.config.virtualUserUserKind = 3;
			...
			//jasmine.getFixtures().fixturesPath = "base/index.html";

			//setFixtures("<body class='bg-light'><h3>App " + dbCode + " - TEST e2e automatici</h3><h5 id='teste2e_mp_id'></h5><div id='toolbar'></div><div class='container' id='metaRoot'></div></body>");
			// importo css per corretta visualizzazione sul browser in questione durante il running del test
			// $("body").append('<link rel="stylesheet" href="styles/italia/bootstrap-italia.min.css" />');
			// $("body").append('<link rel="stylesheet" href="styles/italia/italia-icon-font.css" />');
			// $("body").append('<link rel="stylesheet" href="styles/custom.css" />');
			// $("body").append('<link rel="stylesheet" href="components/styles/app.css" />');
			// $("body").append('<link rel="stylesheet" href="components/styles/jquery.toast.css" />');
			// $("body").append('<link rel="stylesheet" href="components/styles/select2.min.css" />');
		...
		buildFakeMenuWeb: function(tname, edittype) {
			...
			window.appMeta.security.dtMenuWeb = dtMenuWeb;

			// inserisco fake usr_envche matcha con la riga dtMenuWeb
			window.appMeta.security.usr("mw_" + idmenuweb, "'S'");
			window.appMeta.security.usr("mr_" + idmenuweb, "'S'");
		},
		...
		waitEvent: function (eventName) {
			...
			let f = function (sender) {
				window.appMeta.globalEventManager.unsubscribe(eventName, f, that);
				// sender è il sender, ovvero la metaPage che ha lanciato l'evento
				def.resolve(sender);
			};
			window.appMeta.globalEventManager.subscribe(eventName, f, that);
			return def;
		},
		...
		testMetaPageInitialization: function (metaPage, tableName, editType) {
			expect( window.appMeta.currApp.currentMetaPage).toBeDefined();
        ...
        */
         ...
         * @param {string} value
         */
		setInputByTag: function (tag, value) {
			this.getInputByTag(tag).val(value);
		},
		...
		clickButtonByTag: function (tagCmd, detailLog) {
			...
			//console.log("Comando invocato : " + tagCmd + " " + (!!detailLog ? detailLog : ""));
		...
		testHtmlNodeByTagBindedDataSet: function (arrayInput, metapage, treal, talias ,afterCheckTree, recursive) {
			...
			function checkInputText(elInputText) {
				...
				if (dt.rows.length) {
					...
					if (!dt.columns[columnName]) {
						...
						var value = new  window.appMeta.TypedObject(ctype, elInputText.value, completeTag).value;
						if (ctype === "DateTime") {
			...
			function checkInputGridOrCalendar(elInput, elName) {
				...
				if (!isInsertBtnVisible) {
					...
					if (rowsToCount) {
						...
						var myinstance = appMeta[detailTableNameReal + '_' + detailEditType];
											//eval(detailTestPrototype);
						if (!myinstance) {
			...
			function checkTreeCustom(elInput, isRootAndChild) {
				...
				var autoChildRelation = window.appMeta.getDataUtils.getAutoChildRelation(dtTree);
				...
				switch (input.type) {
					...
					case controlTypeEnum.openPage:
						...
						chainFill = chainFill.then(function () {
							return this.waitEvent(window.appMeta.EventEnum.showPage);
						});
						window.appMeta.currApp.callPage(tablename, edittype, false);
						break;

					case controlTypeEnum.buttonInsert:
						chainFill = chainFill.then(function () {
							return common.eventGlobalWaiter(null, window.appMeta.EventEnum.buttonClickEnd);
						});
					...
					case controlTypeEnum.select:
						...
						var fillSelect = function (input) {
							...
							common.pageEventWaiter(helpForm.metaPage, window.appMeta.EventEnum.afterComboChanged)
								.then(function () {
						...
						var fillAutochoose = function ($txt, input) {
							...
							return helpForm.textBoxGotFocus.call($txt, helpForm)
								.then(function () {
									...
									helpForm.metaPage.eventManager.subscribe(window.appMeta.EventEnum.showModalWindow, clickRowOnListManager, self);
									...
									return helpForm.textBoxLostFocus.call($txt, helpForm)
										.then(function () {
											...
											// successivamente
											helpForm.metaPage.eventManager.unsubscribe(window.appMeta.EventEnum.showModalWindow, clickRowOnListManager, self);
											return true;
										})
								});
						};
					...
					case controlTypeEnum.grid:
						...
						var fillgrid = function (tag, rowcount) {
							...
							self.waitEvent(window.appMeta.EventEnum.showPage)
								.then(function (metaPageDetail) {
									...
									var myinstance = window.appMeta[detailTableName + '_' + detailEditType];
									...
									return self.testDatasetCompliant(detailTableName, detailEditType).then(function () {
										...
										// attendo la fine del tasto di add sulla metaPage chiamante
										common.pageEventWaiter(helpForm.metaPage, window.appMeta.EventEnum.insertClick)
											.then(function () {
					...
					case controlTypeEnum.dropdowngrid:
						...
						var fillDropDown = function ($txt, input) {
							...
							helpForm.metaPage.eventManager.subscribe(
								window.appMeta.EventEnum.listCreated,
								clickRowOnListManager, self);
							...
							return customDropDownCtrl.keyup(customDropDownCtrl, false)
								.then(function () {
									...
									// invocherò clickRowOnListManager(), cioè la callback, verrà risolto il deferred ed entreremo in questo ramo then()
									// successivamente
									helpForm.metaPage.eventManager.unsubscribe(
										window.appMeta.EventEnum.listCreated,
										clickRowOnListManager, self);
									return true;
								})
					...
					case controlTypeEnum.treecustom:
						...
						var fillTreeCustom = function (tag) {
							...
							if (treeCustomCtrl.metaPage.state.currentRow.getRow().state === dataRowState.added) {
								...
								if (!window.appMeta.dictTestAuxTree) {
									window.appMeta.dictTestAuxTree = {};
								}
								...
								window.appMeta.dictTestAuxTree[treeTableName] = {
									ctrl: treeCustomCtrl,
									tname: treeTableName,
		...
		getRandomDecimalTest: function (precision) {
			return (Math.random() * (0.1 - 0.9) + 0.9).toFixed(precision).
					replace(".", window.appMeta.numberDecimalSeparator);
		},
		...
		testDatasetCompliant: function (tablename, edittype) {
			var def = window.appMeta.Deferred('testDatasetCompliant');
	...
	window.appMeta.testHelper= new TestHelper();
	window.appMeta.testHelper.controlTypeEnum = controlTypeEnum;
	window.appMeta.testHelper.enumLogType = EnumLogType;
}());
